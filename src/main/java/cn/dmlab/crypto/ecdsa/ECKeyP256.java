 ...
 */
public class ECKeyP256 implements Serializable {

    private static Logger logger = Logger.getLogger(ECKeyP256.class);

    /**
     * The parameters of the secp256r1 curve that BitXHub uses.
     */
     ...
     */
    public ECKeyP256(Provider provider, SecureRandom secureRandom) {
        this.provider = provider;

        final KeyPairGenerator keyPairGen = ECKeyPairGenerator.getInstance(provider, secureRandom, CURVE_NAME);
     ...
     * @param secureRandom -
     */
    public ECKeyP256(SecureRandom secureRandom) {
     ...
     * All private key operations will use the provider.
     */
    public ECKeyP256(Provider provider, PrivateKey privKey, ECPoint pub) {
        this.provider = provider;
     ...
     * BouncyCastle will be used as the Java Security Provider
     */
    public ECKeyP256(BigInteger priv, ECPoint pub) {
        this(
     ...
     * @return -
     */
    public static ECKeyP256 fromPrivate(BigInteger privKey) {
        return new ECKeyP256(privKey, CURVE.getG().multiply(privKey));
    }
     ...
     * @return -
     */
    public static ECKeyP256 fromPrivate(byte[] privKeyBytes) {
     ...
     * @return -
     */
    public static ECKeyP256 fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new ECKeyP256(priv, pub);
    }
     ...
     * @return -
     */
    public static ECKeyP256 fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        check(priv != null, "Private key must not be null");
        check(pub != null, "Public key must not be null");
        return new ECKeyP256(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @return -
     */
    public static ECKeyP256 fromPublicOnly(ECPoint pub) {
        return new ECKeyP256(null, pub);
    }
     ...
     * @return -
     */
    public static ECKeyP256 fromPublicOnly(byte[] pub) {
        return new ECKeyP256(null, CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyP256 decompress() {
        if (!pub.isCompressed())
            return this;
        else
            return new ECKeyP256(this.provider, this.privKey, decompressPoint(pub));
    }

    /**
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyP256 compress() {
        if (pub.isCompressed())
            return this;
        else
            return new ECKeyP256(this.provider, this.privKey, compressPoint(pub));
    }
    ...
    public static class ECDSASignature {
        ...
        public static ECDSASignature decode(byte[] signatureEncoded) {
            byte[] r = Arrays.copyOfRange(signatureEncoded, 0, 32);
            byte[] s = Arrays.copyOfRange(signatureEncoded, 32, 64);
            byte v = (byte) (signatureEncoded[64] & 0xFF);
        ...
        public String toHex(byte[] pubKey) {
            return Hex.toHexString(toByteArray(pubKey));
        }
    ...
    public ECDSASignature sign(byte[] messageHash) {
        ...
        for (int i = 0; i < 4; i++) {
            byte[] k = ECKeyP256.recoverPubBytesFromSignature(i, sig, messageHash);
    ...
    public static byte[] signatureToKeyBytes(byte[] messageHash, ECDSASignature sig) throws SignatureException {
        ...
        int recId = header - 27;
        byte[] key = ECKeyP256.recoverPubBytesFromSignature(recId, sig, messageHash);
        if (key == null)
     ...
     * @return ECKey
     */
    public static ECKeyP256 signatureToKey(byte[] messageHash, String signatureBase64) throws SignatureException {
        final byte[] keyBytes = signatureToKeyBytes(messageHash, signatureBase64);
        return ECKeyP256.fromPublicOnly(keyBytes);
    }
     ...
     * @return ECKey
     */
    public static ECKeyP256 signatureToKey(byte[] messageHash, ECDSASignature sig) throws SignatureException {
    ...
    public boolean verify(byte[] data, byte[] signature) {
        return ECKeyP256.verify(data, signature, getPubKey());
    }
    ...
    public boolean verify(byte[] sigHash, ECDSASignature signature) {
        return ECKeyP256.verify(sigHash, signature, getPubKey());
    }
     ...
     * @return ECKey
     */
    public static ECKeyP256 recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig, messageHash);
        if (pubBytes == null) {
            return null;
        } else {
            return ECKeyP256.fromPublicOnly(pubBytes);
        }
    }
    ...
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || !(o instanceof ECKeyP256)) return false;

        ECKeyP256 ecKey = (ECKeyP256) o;
