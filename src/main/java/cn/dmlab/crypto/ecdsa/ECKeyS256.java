 ...
 */
public class ECKeyS256 implements Serializable {

    private static Logger logger = Logger.getLogger(ECKeyS256.class);
     ...
     */
    public ECKeyS256(Provider provider, SecureRandom secureRandom) {
     ...
     * @param secureRandom -
     */
    public ECKeyS256(SecureRandom secureRandom) {
     ...
     * All private key operations will use the provider.
     */
    public ECKeyS256(Provider provider, PrivateKey privKey, ECPoint pub) {
        this.provider = provider;
     ...
     * BouncyCastle will be used as the Java Security Provider
     */
    public ECKeyS256(BigInteger priv, ECPoint pub) {
        this(
     ...
     * @return -
     */
    public static ECKeyS256 fromPrivate(BigInteger privKey) {
        return new ECKeyS256(privKey, CURVE.getG().multiply(privKey));
    }
     ...
     * @return -
     */
    public static ECKeyS256 fromPrivate(byte[] privKeyBytes) {
     ...
     * @return -
     */
    public static ECKeyS256 fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new ECKeyS256(priv, pub);
    }
     ...
     * @return -
     */
    public static ECKeyS256 fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        check(priv != null, "Private key must not be null");
        check(pub != null, "Public key must not be null");
        return new ECKeyS256(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @return -
     */
    public static ECKeyS256 fromPublicOnly(ECPoint pub) {
        return new ECKeyS256(null, pub);
    }
     ...
     * @return -
     */
    public static ECKeyS256 fromPublicOnly(byte[] pub) {
        return new ECKeyS256(null, CURVE.getCurve().decodePoint(pub));
    }
     ...
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyS256 decompress() {
        if (!pub.isCompressed())
            return this;
        else
            return new ECKeyS256(this.provider, this.privKey, decompressPoint(pub));
    }

    /**
     * @deprecated per-point compression property will be removed in Bouncy Castle
     */
    public ECKeyS256 compress() {
        if (pub.isCompressed())
            return this;
        else
            return new ECKeyS256(this.provider, this.privKey, compressPoint(pub));
    }
    ...
    public static class ECDSASignature {
        ...
        public static boolean validateComponents(BigInteger r, BigInteger s, byte v) {
            ...
            if (!BIUtil.isLessThan(r, SECP256K1N)) return false;
            if (!BIUtil.isLessThan(s, SECP256K1N)) return false;
    ...
    public ECDSASignature sign(byte[] messageHash) {
        ...
        for (int i = 0; i < 4; i++) {
            byte[] k = ECKeyS256.recoverPubBytesFromSignature(i, sig, messageHash);
    ...
    public static byte[] signatureToKeyBytes(byte[] messageHash, ECDSASignature sig) throws SignatureException {
        ...
        int recId = header - 27;
        byte[] key = ECKeyS256.recoverPubBytesFromSignature(recId, sig, messageHash);
        if (key == null)
     ...
     * @return ECKey
     */
    public static ECKeyS256 signatureToKey(byte[] messageHash, String signatureBase64) throws SignatureException {
        final byte[] keyBytes = signatureToKeyBytes(messageHash, signatureBase64);
        return ECKeyS256.fromPublicOnly(keyBytes);
    }
     ...
     * @return ECKey
     */
    public static ECKeyS256 signatureToKey(byte[] messageHash, ECDSASignature sig) throws SignatureException {
    ...
    public boolean verify(byte[] data, byte[] signature) {
        return ECKeyS256.verify(data, signature, getPubKey());
    }
    ...
    public boolean verify(byte[] sigHash, ECDSASignature signature) {
        return ECKeyS256.verify(sigHash, signature, getPubKey());
    }
     ...
     * @return ECKey
     */
    public static ECKeyS256 recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash) {
        final byte[] pubBytes = recoverPubBytesFromSignature(recId, sig, messageHash);
        if (pubBytes == null) {
            return null;
        } else {
            return ECKeyS256.fromPublicOnly(pubBytes);
        }
    }
    ...
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || !(o instanceof ECKeyS256)) return false;

        ECKeyS256 ecKeyS256 = (ECKeyS256) o;

        if (privKey != null && !privKey.equals(ecKeyS256.privKey)) return false;
        if (pub != null && !pub.equals(ecKeyS256.pub)) return false;
